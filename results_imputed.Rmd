...
VolcanoPlot(coef=modfit_nona$coefficients[,1], pvals=modfit_nona$p.value[,1],plimit=0.05,
            interactiveonly = FALSE)

test <- NormalizeMets::LinearModelFit(featuredata = data.matrix(dietmat_nounk$featuredata-nodietmat_nounk$featuredata),
                                      ruv2=FALSE,
                                      factormat=matrix(1,nrow=nrow(dietmat_nounk$featuredata)),
                                      saveoutput = FALSE)
...
tab1 <- print(tab1,varLabels=TRUE)
...
dev.new()
cim(splsda.diet, row.sideColors = color.mixo(as.factor(nomiss.plsda$Group)), row.names = FALSE, col.names=FALSE,
...
for (i in 1:nrow(fourgroup.nm$groupdata)) {
   fourgroup.nm$groupdata$PCOS[i] <- ifelse(row.names(fourgroup.nm$groupdata)[i] %in% pcos,1,0)
}
...
plot4group <- fviz_pca_ind(res4group.pca,
             geom.ind = "point", # show points only (nbut not "text")
             col.ind = as.factor(fourgroup.nm$groupdata$four), # color by groups
             palette = c("#00AFBB", "#E7B800"),
             addEllipses = TRUE, # Concentration ellipse
             title=""
             )
...
# STOPPED HERE - NEED TO MAKE SURE CODE BELOW IS CALLING CORRECT OBJECTS AND CUT AND PASTE NORMALIZE METS CODE
