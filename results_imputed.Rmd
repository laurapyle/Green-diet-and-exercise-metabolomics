...
write.csv(dietmat_nounk$featuredata-nodietmat_nounk$featuredata,"H:\\Endocrinology\\Green\\Metabolomics papers\\Diet and exercise\\Data\\diet minus nodiet.csv")
...
p <- ggplot(data=sig_ord, aes(x=X, y=coeff.x1,ymin=Lower.CI.x1,ymax=Upper.CI.x1)) + 
  ...
  coord_flip() + xlab("") + ylab("Mean Log Difference (Diet-No Diet)") +
...
VolcanoPlot(coef=modfit_nona$coefficients[,1], pvals=modfit_nona$adj.p.value[,1],plimit=0.05,interactiveonly = FALSE,ylab="-log(q-value)",labelsig = FALSE, fclabel = "q<0.05")
...
cim(splsda.diet30, row.sideColors = color.mixo(as.factor(nomiss.plsda$Group)), row.names = FALSE, col.names=TRUE,
    legend=list(legend=c("No Diet","Diet")),
    save="jpeg",
...
gooddata.pcos$Visit <- gooddata.pcos$Group
...
splsda.pcos.diet = splsda(X = nomiss.plsda_pcos[nomiss.plsda_pcos$Visit==1,c(4:6694)], Y=as.factor(nomiss.plsda_pcos[nomiss.plsda_pcos$Visit==1,]$PCOS),
...
# try sPLS-DA for PCOS vs. control in diet and fasting samples separately
splsda.pcos.nodiet = splsda(X = nomiss.plsda_pcos[nomiss.plsda_pcos$Visit==0,c(4:6694)], Y=as.factor(nomiss.plsda_pcos[nomiss.plsda_pcos$Visit==0,]$PCOS),
                     ncomp = 2, max.iter = 100000,near.zero.var=TRUE,tol=0.1)
...
VolcanoPlot(coef=modfit_nona$coefficients[,1], pvals=modfit_nona$adj.p.value[,1],plimit=0.05,interactiveonly = FALSE,ylab="-log(q-value)",labelsig = FALSE)
```
...
3-fold cross-validation was used to examine the performance of the sPLS-DA model based on PCOS vs. control, using all metabolites, clamp visit samples only. 

 Overall Error rate : `r pcos.perf.splsda.diet$error.rate`
 AUC: `r pcos.perf.splsda.diet$auc`
...
3-fold cross-validation was used to examine the performance of the sPLS-DA model based on PCOS vs. control, using all metabolites, fasting visit samples only. 

 Overall Error rate : `r pcos.perf.splsda.nodiet$error.rate`
 AUC: `r pcos.perf.splsda.nodiet$auc`
