...
final_nomissqrilc <- MissingValues(featuredata = final$featuredata,sampledata = final$sampledata,
                                 metabolitedata=final$metdata,feature.cutoff=0.8,method="none",
                                 seed=3654, saveoutput = TRUE,
                                 outputname = "C:\\Temp\\final_nomissqrilc")
# now we should have an object with featuredata, sampledata, and groupdata
# impute with qrilc
temp <- impute.QRILC(final_nomissqrilc$featuredata)
final_nomissqrilc$featuredata <- temp[[1]]

# write imputed dataset for reading into PLS-DA
write.csv(final_nomissqrilc$featuredata,"H:\\Endocrinology\\Green\\Metabolomics papers\\Diet and exercise\\Data\\qrilc_imputed_for_plsda.csv")

# make a version with only knowns
final_nomissqrilc_nounk <- NULL
final_nomissqrilc_nounk$featuredata <- final_nomissqrilc$featuredata[, -grep("UNK",colnames(final_nomissqrilc$featuredata))]
final_nomissqrilc_nounk$sampledata <- final_nomissqrilc$sampledata
final_nomissqrilc_nounk$metabolitedata <- final_nomissqrilc$metabolitedata[-grep("UNK",final_nomissqrilc$metabolitedata)]

# log transform both the entire dataset and the known dataset
final_nomissqrilc.log <- NULL
temp <- final_nomissqrilc$featuredata
final_nomissqrilc.log <- LogTransform(temp)
final_nomissqrilc.log$sampledata <- final_nomissqrilc$sampledata
final_nomissqrilc.log$metabolitedata <- final_nomissqrilc$metabolitedata
final_nomissqrilc_nounk.log <- NULL
temp <- final_nomissqrilc_nounk$featuredata
final_nomissqrilc_nounk.log <- LogTransform(temp)
final_nomissqrilc_nounk.log$sampledata <- final_nomissqrilc_nounk$sampledata
final_nomissqrilc_nounk.log$metabolitedata <- final_nomissqrilc_nounk$metabolitedata

#Linear model fit with moderated statistics
modFit<-LinearModelFit(featuredata=final_nomissqrilc_nounk.log$featuredata,
                       ruv2=FALSE,
                       moderated=TRUE,
                       factormat=final_nomissqrilc_nounk.log$sampledata[1],
                       outputname = "H:\\Endocrinology\\Green\\Metabolomics papers\\Diet and exercise\\Data\\modFit_qrilc",
                       saveoutput = TRUE)

# BoxPlots for significant compounds
sig <- read.csv(file="H:\\Endocrinology\\Green\\Metabolomics papers\\Diet and exercise\\Data\\modFit_qrilc.csv")
...
res.pca <- PCA(final_nomissqrilc$featuredata, graph = FALSE)
png("C:\\temp\\pca_qrilc.png")
plot1 <- fviz_pca_ind(res.pca,
             geom.ind = "point", # show points only (nbut not "text")
             col.ind = as.factor(final_nomissqrilc$sampledata$Group), # color by groups
...
nomissdf_pcos <- read.csv("H:\\Endocrinology\\Green\\Metabolomics papers\\Diet and exercise\\Data\\qrilc_imputed_for_plsda.csv")
...
```{r, echo=FALSE}
knitr::include_graphics("C:\\temp\\pca_qrilc.png")
```
