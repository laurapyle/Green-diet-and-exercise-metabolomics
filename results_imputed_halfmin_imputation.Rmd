...
# now impute by PCOS
final_nomissknn <- MissingValues(featuredata = final$featuredata,sampledata = final$sampledata,
                                 metabolitedata=final$metdata,feature.cutoff=0.8,method="knn",
                                 featuremax.knn = 0.99,seed=3654, saveoutput = TRUE,
...
final_nomissknn_nounk <- NULL
final_nomissknn_nounk$featuredata <- final_nomissknn$featuredata[, -grep("UNK",colnames(final_nomissknn$featuredata))]
final_nomissknn_nounk$sampledata <- final_nomissknn$sampledata
final_nomissknn_nounk$metabolitedata <- final_nomissknn$metabolitedata[-grep("UNK",final_nomissknn$metabolitedata)]

# log transform both the entire dataset and the known dataset
final_nomissknn.log <- NULL
temp <- final_nomissknn$featuredata
final_nomissknn.log <- LogTransform(temp)
final_nomissknn.log$sampledata <- final_nomissknn$sampledata
final_nomissknn.log$metabolitedata <- final_nomissknn$metabolitedata
final_nomissknn_nounk.log <- NULL
temp <- final_nomissknn_nounk$featuredata
final_nomissknn_nounk.log <- LogTransform(temp)
final_nomissknn_nounk.log$sampledata <- final_nomissknn_nounk$sampledata
final_nomissknn_nounk.log$metabolitedata <- final_nomissknn_nounk$metabolitedata

#Linear model fit with moderated statistics
modFit<-LinearModelFit(featuredata=final_nomissknn_nounk.log$featuredata,
                       ruv2=FALSE,
                       moderated=TRUE,
                       factormat=final_nomissknn_nounk.log$sampledata[1],
                       outputname = "H:\\Endocrinology\\Green\\Metabolomics papers\\Diet and exercise\\Data\\modFit_knn",
                       saveoutput = TRUE)

# BoxPlots for significant compounds
sig <- read.csv(file="H:\\Endocrinology\\Green\\Metabolomics papers\\Diet and exercise\\Data\\modFit_knn.csv")
...
res.pca <- PCA(final_nomissknn$featuredata, graph = FALSE)
png("C:\\temp\\pca_knn.png")
plot1 <- fviz_pca_ind(res.pca,
             geom.ind = "point", # show points only (nbut not "text")
             col.ind = as.factor(final_nomissknn$sampledata$Group), # color by groups
...
# PLS-DA
nomissdf_pcos <- fread("C:\\Temp\\final_nomissknn_featuredata.csv",header=TRUE)
...
nomissdf_pcos <- cbind(nomissdf_pcos[,6498],nomissdf_pcos[1:6497])
colnames(nomissdf_pcos) <- c("PCOS",colnames(nomissdf_pcos[,2:6498]))
...
```{r, echo=FALSE}
knitr::include_graphics("C:\\temp\\pca_knn.png")
```
