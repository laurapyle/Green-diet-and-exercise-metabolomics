...
p <- ggplot(data=sig_ord, aes(x=X, y=coeff.x1,ymin=Lower.CI.x1,ymax=Upper.CI.x1)) + 
  ...
  coord_flip() + xlab("") + ylab("Relative Log Difference (Diet-No Diet) and 95% CI") +
...
splsda.diet = splsda(X = nomiss.plsda[,c(4:168)], Y=as.factor(nomiss.plsda$Group), 
                   ncomp = 3, multilevel = as.factor(nomiss.plsda$id))
plotIndiv(splsda.diet, comp = c(1,2,3),
          ind.names = FALSE, 
          ellipse = TRUE, legend = TRUE, title="",style="3d")
...
NormalizeMets::VolcanoPlot(coef=modfit_nona$coefficients[,1], pvals=modfit_nona$p.value[,1],plimit=0.05,
            interactiveonly = FALSE)
```
